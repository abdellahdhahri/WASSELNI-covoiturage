...
<template>
 <div v-if="trajetres.length>0">
  <div class="rech" style="margin-top: 40px; margin-bottom: 40px;margin-left: 20px;" v-for="trajet in trajetres" :key="trajet.id">
    <div class="card">
  <div class="img">
    <div class="save">
...
<g clip-path="url(#clip0_993_25)">
  ...
  <div class="text">
    <p class="h3"> {{ trajet.lieu_depart }} To {{ trajet.lieu_arrive }} </p>
    <p class="p"> {{trajet.temps_depart}} </p>
...
<g>
      ...
      <p class="span">{{trajet.vehicule}}
        
    </p><button type="button" class="btn btn-primary" style="width: 80px; margin-top: 70px;position: relative; margin-left: 600px;">Reserve</button>
    <router-link :to="{ name: 'detailTrajets', params: { id: trajet.id } }"><button type="button" class="btn btn-primary" style="width: 80px; margin-top: 70px;position: relative; margin-left: 20px;">Detail</button></router-link>
</div>
...
<div class="notfound" style="height: 500px;width: 100%;margin-top: 30px; display: flex;align-items: center;
justify-content: center; margin-top: 40px;" v-else>
...
onMounted(() => {
  fetchtrajet();
  if (route.params.trajetres) {
        trajetres.value = JSON.parse(route.params.trajetres);
      }
});
console.log('resultttttttttt',trajetres.value)
</script>
<style scoped>
.card {
  width:100%;
...
.img {
  ...
  background: linear-gradient(#e7d105, #9198e5);
  display: flex;
  align-items: top;
